<template>
  <div>
    <div class="flex flex-col md:flex-row justify-between items-end md:items-center mb-10 gap-4">
      <div>
        <h1 class="text-4xl font-extrabold text-[#1A1A1A]">Dashboard Lektora</h1>
        <p class="text-gray-600 font-semibold mt-2">Spravuj své kurzy a materiály na jednom místě.</p>
      </div>
      <button 
        @click="openModal()" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
      <div class="organic-box p-6 !bg-[#0070BB] text-white !border-[#0070BB]">
        <h3 class="text-lg font-bold opacity-90 mb-2">Aktivní kurzy</h3>
        <div class="text-5xl font-black">{{ courses.length }}</div>
      </div>
      <div class="organic-box p-6 bg-white">
        <h3 class="text-lg font-bold text-gray-500 mb-2">Studenti celkem</h3>
        <div class="text-5xl font-black text-[#1A1A1A]">842</div>
      </div>
      <div class="organic-box p-6 bg-white">
        <h3 class="text-lg font-bold text-gray-500 mb-2">Hodnocení</h3>
        <div class="text-5xl font-black text-[#FFD93D]">4.9</div>
      </div>
    </div>

    <h3 class="text-2xl font-bold mb-6 border-b-2 border-gray-200 pb-2">Moje Kurzy</h3>
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      >
        <div class="w-24 h-24 rounded-lg flex-shrink-0 border-2 border-[#1A1A1A] bg-gray-100 relative overflow-hidden">
          <div 
...
const fetchCourses = async () => {
  try {
    ...
    courses.value = data.map((course: Course, index: number) => ({
      ...course,
      difficulty: course.difficulty || ['Začátečník', 'Pokročilý', 'Expert'][index % 3],
...
const deleteCourse = async (uuid?: string) => {
  if (!uuid) {
    alert('Chyba: Kurz nemá platné ID')
    return
  }
  ...
  try {
    const response = await fetch(`/api/courses/${uuid}`, {
      method: 'DELETE'
    })
