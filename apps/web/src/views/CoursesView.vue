  ...
  <div>
    <div class="flex flex-col md:flex-row gap-8 mb-12">
      <aside class="md:w-1/4 space-y-8 sticky top-24 h-fit">
        ...
        <div>
          <h3 class="text-2xl font-bold text-[#0257A5] mb-4 uppercase -rotate-2">Kategorie</h3>
          <div class="space-y-2">
            <div 
              ...
              :class="{ 'bg-[#91F5AD]/30 text-[#0257A5] translate-x-2': activeCategory === cat }"
            >
              {{ cat }}
            </div>
          </div>
      ...
      <div class="md:w-3/4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 pb-12">
        ...
        <div v-else-if="error" class="col-span-full text-center text-red-500 font-bold">
          {{ error }}
        </div>
        <CourseCard
          v-else
...
const filteredCourses = computed(() => {
  return courses.value.filter(c => {
    ...
    const matchesCat = activeCategory.value === 'Všechny' || c.category === activeCategory.value
...
const viewCourse = (course: Course) => {
  ...
  // TODO: Navigate to course detail or open modal
}
