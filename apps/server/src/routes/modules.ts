...
modulesRouter.post("/", async (req, res) => {
  ...
  try {
    ...
    await pool.execute(
      ...
      [uuid, (courses as any)[0].id, title, description || null, order_index || 0, false]
    );
...
// Úprava modulu (publikování/skrytí atd.)
  ...
  try {
    const [result] = await pool.execute(
      `UPDATE modules 
       ...
       WHERE uuid = ? OR id = ?`,
      [title, description, is_published, order_index, moduleId, moduleId]
    );

    if ((result as any).affectedRows === 0) return res.status(404).json({ error: "Modul nenalezen" });
